...
export default function UpdateContact({ contactId, user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateContactForm(formData)) {
      setIsLoading(true);
      const contactUpdated = await updateContact(formData, user, contactId);
      if (contactUpdated) setIsLoading(false);
      router.push("/contact");
      setIsLoading(false);
    }
  };
    ...
    <div className="flex flex-col items-center justify-center py-6 bg-gray-100">
      <div className="max-w-md w-full py-5 px-8">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-800">Update Contact</h2>
        </div>
